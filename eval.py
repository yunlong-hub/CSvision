...
# from mit_semseg.all_dataset import val_dataset
from mit_semseg.dataset import val_dataset

colors = loadmat('/data/yunlong/Project/CSvision/Data/ADE20K/color150.mat')['colors']
...
def worker(cfg, gpu_id, start_idx, end_idx, result_queue):
    ...
    # Dataset and Loader
    # dataset_val = ADE_ValDataset(
    #     cfg.DATASET.root_dataset,
    #     cfg.DATASET.list_val,
    #     cfg.DATASET,
    #     start_idx=start_idx, end_idx=end_idx)

    dataset_val = val_dataset(cfg, start_idx, end_idx)
...
def main(cfg, gpus,history):
    ...
    history['val']['Mean IoU'].append(iou.mean())  # 将损失添加到训练历史
    history['val']['acc'].append(acc_meter.average()*100)  # 将准确率添加到训练历史
...
if __name__ == '__main__':
    ...
    val_history = {'val': {'epoch': [], 'Mean IoU': [], 'acc': []}}
    for i in range(1,11):
        VAL_checkpoint = "epoch_{}.pth".format(i)
        # absolute paths of model weights
        cfg.MODEL.weights_encoder = os.path.join(
            cfg.DIR, 'encoder_' + VAL_checkpoint)
        cfg.MODEL.weights_decoder = os.path.join(
            cfg.DIR, 'decoder_' + VAL_checkpoint)
        assert os.path.exists(cfg.MODEL.weights_encoder) and \
            os.path.exists(cfg.MODEL.weights_decoder), "checkpoint does not exitst!"

        if not os.path.isdir(os.path.join(cfg.DIR, "result")):
            os.makedirs(os.path.join(cfg.DIR, "result"))
        print("第{}轮模型有效性验证：".format(i))
        # Parse gpu ids
        gpus = parse_devices(args.gpus)
        gpus = [x.replace('gpu', '') for x in gpus]
        gpus = [int(x) for x in gpus]
        main(cfg, gpus,val_history)
        val_history['val']['epoch'].append(str(i))  # 将epoch添加到训练历史

    torch.save(val_history, '{}/val_history.pth'.format(cfg.DIR))
